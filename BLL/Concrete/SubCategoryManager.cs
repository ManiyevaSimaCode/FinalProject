...
namespace BLL.Concrete
{
    public class SubCategoryManager : ISubCategoryService
    {
        ...
        {
            SubCategory subcategory = await _unitOfWork.SubCategoryRepository.GetAsync(c => c.Id == id && !c.isDeleted, "SubCategoryParameters.Parameter");
            
            subcategory.ParameterIds = subcategory.SubCategoryParameters.Select(c => c.ParameterId).ToList();
            subCategoryPostDto.SubCategoryParameters.RemoveAll(sb => !subcategory.ParameterIds.Contains(sb.ParameterId));
            foreach ( var parameterId in subcategory.ParameterIds
                .Where(x=>!subCategoryPostDto.SubCategoryParameters.Any(sb=>sb.ParameterId==x)))
            {
